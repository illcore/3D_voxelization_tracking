...
if(Volume->GetName() == "PhanVoxels"){
CheckFile.open("Proton_tracking_phantom.dat", std::ios::app);
CheckFile << EventID << " " << PosX << " " << PosY << " " << PosZ << " " << KineticEnergy << " " << MomentumMagnitude << " " << Volume->GetName() << " " << CopyNumber << " " << ParticleName << " " << ProcessName <<  " " << depositedEnergy << G4endl;
CheckFile.close();   
}
}
}
